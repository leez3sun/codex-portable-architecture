...
def launch_workbench_journal(
    ...
    if batch:
        ...
        # Be explicit for teaching runs. Workbench documents -I as interactive
        # mode and -R as replaying the journal at startup.
        cmd.append("-I")
    ...
    # Batch jobs should not flash a console window. Visible training jobs must
    # keep normal Windows process creation semantics so Workbench and any
    # geometry editor it launches can expose foreground windows.
    creationflags = getattr(subprocess, "CREATE_NO_WINDOW", 0) if batch else 0
